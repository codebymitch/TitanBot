...
export default { data: new SlashCommandBuilder().setName('settings').setDescription('Manage bot settings').setDefaultMemberPermissions(PermissionFlagsBits.ManageGuild)
  ...
  async execute(interaction, client) { const sub = interaction.options.getSubcommand(); if (sub === 'reset') { await resetConfig(client, interaction.guildId); return interaction.reply({...success('ההגדרות אופסו', 'הבוט חזר להגדרות ברירת המחדל.'),flags:MessageFlags.Ephemeral}); } const c = await getConfig(client, interaction.guildId); return interaction.reply({...info('הגדרות השרת', `מערכת קבלת פנים: **${c.welcome.enabled ? 'פעילה' : 'כבויה'}**\nאימות: **${c.verification.enabled ? 'פעיל' : 'כבוי'}**\nפניות: **${c.tickets.enabled ? 'פעילות' : 'כבויות'}**\nלוגים: **${c.logging.enabled ? 'פעילים' : 'כבויים'}**\nרמות: **${c.leveling.enabled ? 'פעילות' : 'כבויות'}**`),flags:MessageFlags.Ephemeral}); } };
