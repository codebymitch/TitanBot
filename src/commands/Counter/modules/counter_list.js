...
export default {
    async execute(interaction, config, client) {
        try {
                ...
                return interaction.editReply({
                    embeds: [
                        createEmbed({ title: "No Counters", description: BotConfig.counters.messages.noCounters ||
                                "There are no active counters in this server. Use `/counter create` to create one." }),
                    ],
                });
            }
            ...
            console.error('Counter list command error:', error);
            return interaction.reply({
                embeds: [errorEmbed('System Error', 'Could not list counters at this time.')],
                ephemeral: true,
            });
        }
    }
