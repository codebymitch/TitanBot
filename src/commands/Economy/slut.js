...
export default {
    ...
    data: new SlashCommandBuilder()
        .setName("slut")
        .setDescription("Engage in adult activities for money (18+)")
        .addStringOption(option =>
            option
                .setName("activity")
                .setDescription("Type of activity to engage in")
                .setRequired(false)
                .addChoices(
    ...
    // Prefix command data
    name: "slut",
    aliases: ["hooker", "escort", "prostitute"],
    description: "Engage in adult activities for money (18+)",
    category: "Economy",
    usage: `${botConfig.commands.prefix}slut [activity]`,
                ...
                return interaction.editReply({
                    embeds: [
                        errorEmbed(
                            ...
                            `${response}\n\n` +
                            `💳 **Current Balance:** ${userData.balance.toLocaleString()} coins\n` +
                            `📊 **Total Sessions:** ${userData.totalSluts}\n` +
    ...
    async prefixExecute(message, args, config, client) {
        ...
        try {
            ...
            if (args[0]) {
                ...
                activity = SLUT_ACTIVITIES.find(a => 
                    a.name.toLowerCase().includes(activityName) || 
                    activityName.includes(a.name.toLowerCase())
                );
            }
            
            if (!activity) {
                activity = SLUT_ACTIVITIES[Math.floor(Math.random() * SLUT_ACTIVITIES.length)];
            }
